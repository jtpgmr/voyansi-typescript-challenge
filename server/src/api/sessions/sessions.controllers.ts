...
import { generateUserSessionService } from './sessions.services';
import log from '../../utils/logger';

export const generateUserSessionController = async (req: Request, res: Response) => {
  // validate user password input
  const user = await validateUserPasswordService(req.body);
  log.info(config.get("accessTokenPrivateKey"))
